<?php
class Product_DamagedstockController extends Zend_Controller_Action
{
...
public function init()
    ...
    {   
    	if($this->getRequest()->isPost()){   
    		...
    		$db_adjust= new Product_Model_DbTable_DbDamagedStock();
    	...
    	//for add location
    	$frm = new Product_Form_FrmDamagedProduct();
    	Application_Model_Decorator::removeAllDecorator($frm);
...
// 		$sql .= " GROUP BY p.pro_id,pt.type_id";
	...
	public function getProductAction(){
		if($this->getRequest()->isPost()) {
			$db = new Product_Model_DbTable_DbDamagedStock();
			$data = $this->getRequest()->getPost();
			$rs = $db->getProductQtyById($data["id"],$data["location_id"]);

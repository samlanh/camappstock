...
public function add($data=null) {
		...
		$date_order->setAttribs(array(
				'dojoType'=>"dijit.form.DateTextBox",
				'class'=>'fullside',
				'constraints'=>"{datePattern:'dd/MM/yyyy'}",
				));
		$date_order->setValue($date->get(('Y-m-d')));
		...
		$date_in->setValue($date->get(('Y-m-d')));
    	$this->addElement($date_in);
		
		$date_recieve = new Zend_Form_Element_Text('date_recieve');
		$date_recieve->setAttribs(array(
			'dojoType'=>"dijit.form.DateTextBox",
			'class'=>'fullside',
			'constraints'=>"{datePattern:'dd/MM/yyyy'}",));
		$date_recieve->setValue($date->get(('Y-m-d')));
    	...
    	$allTotalElement_after->setAttribs(array(
    			"class"=>"form-control",
    	...
    	$discountValueElement = new Zend_Form_Element_Hidden('discount_value');
    	$discountValueElement->setAttribs(array(
    			'class'=>'input100px','onblur'=>'doTotal();',
    			'style'=>'text-align:right'));
    	$this->addElement($discountValueElement);
    	...
    	$totalAmountAfterElement->setAttribs(array(
    			'readonly'=>'readonly',
    			'style'=>'text-align:right',"class"=>"form-control"
    	));
    	...
    	$remainlElement->setAttribs(array(
    			'readonly'=>'readonly',
    			'style'=>'text-align:right',
    	...
    	$remain_after->setAttribs(array(
    			'readonly'=>'readonly',
    			'style'=>'text-align:right',"class"=>"red form-control"));
    	...
    	$balance_after->setAttribs(array('readonly'=>'readonly',
    			'style'=>'text-align:right',"class"=>"form-control"));
    	...
    	$paidElement->setAttribs(array(
    			'class'=>'custom[number] form-control',
    			'onkeyup'=>'doRemain();','style'=>'text-align:right'));
    	...
    	if($data != null) {
		  ...
		  $date_order->setValue(date("Y-m-d",strtotime($data["date_order"])));
		  $date_in->setValue(date("Y-m-d",strtotime($data["date_in"])));
